...
Running `.\stack-windows.ps1 start` failed during the MariaDB wait phase with:

```text
[INFO ] Waiting for MariaDB in 'koha-docker-windows-db-1'...
docker.exe : ERROR 2002 (HY000): Can't connect to local server through socket '/run/mysqld/mysqld.sock' (2)
At C:\...\stack-windows.ps1:165 char:9
...
`$ErrorActionPreference = "Stop"` is set at the top of the script. In PowerShell 5.1,
any native command that writes to stderr produces a `NativeCommandError` error record.
With `EAP=Stop` that error record is promoted to a **terminating** exception.

The readiness probe (`Get-DbRootMysqlArgs`) ran:

```powershell
& docker exec $DbContainer mysql ... 1>$null 2>$null
```

The `2>$null` redirect suppresses the output stream but does **not** suppress the
`NativeCommandError` object, which is generated by PowerShell's error pipeline *before*
the stream redirect takes effect. A subsequent attempt using `$null = & ... 2>&1` also
failed for the same reason: the error record is created before the merge.

### Fix applied

Wrapped both probe calls in `try/catch` blocks. An empty catch is correct here because:
- A non-zero `$LASTEXITCODE` already communicates "not ready" through the normal
  `return @()` path.
- `EAP=Stop` continues to protect all other failures in the script.

```powershell
# Before (two variants — both fail)
...
try {
    $null = & docker exec $DbContainer mysql -uroot "-p$password" -Nse "SELECT 1" 2>&1
    if ($LASTEXITCODE -eq 0) { return @("-uroot", "-p$password") }
} catch {
...
| File | Change |
|---|---|
| `stack-windows.ps1` | `Get-DbRootMysqlArgs`: both probe calls wrapped in `try/catch` |

---

## 2026-05-05 — Fix StrictMode $null.Count: MariaDB readiness call sites

### Issue observed

After the `NativeCommandError` fix, the script progressed further but then raised:

```text
The property 'Count' cannot be found on this object. Verify that the property exists.
At C:\...\stack-windows.ps1:198 char:13
...
PowerShell 5.1 **unwraps** a function return value of `@()` (empty array) to `$null` at
the call site. With `Set-StrictMode -Version Latest`, accessing `.Count` on `$null`
throws `PropertyNotFoundStrict`.

Three call sites stored the result without `@()` wrapping:

| Location | Code |
|---|---|
| `Wait-DbReady` loop | `$rootArgs = Get-DbRootMysqlArgs ...` |
| `Reset-KohaDatabase` | `$rootArgs = Get-DbRootMysqlArgs ...` |
| `Invoke-HealthCheck` | `(Get-DbRootMysqlArgs ...).Count -gt 0` |

### Fix applied

All three call sites now wrap the function call in `@()` to force array context,
ensuring `.Count` is always available regardless of whether the function returns
...
### Pester test suite added

A Pester 5 test suite was created at `tests/stack-windows.Tests.ps1` covering:

| Describe block | Tests |
|---|---|
| `NativeCommandError regression` | Proves broken impl throws; proves fixed impl does not |
| `Get-DbRootMysqlArgs` | Empty-array return (no throw, Count=0); password auth; passwordless auth; fallback path; exact docker call count |
| `StrictMode $null.Count regression` | Documents PS 5.1 unwrapping; proves `@()` wrapping prevents the throw in all states |
| `Wait-DbReady` | Retries until ready; throws on timeout |
...
Invoke-Pester .\tests\stack-windows.Tests.ps1 -Output Detailed
```
...
| File | Change |
|---|---|
| `stack-windows.ps1` | All three `Get-DbRootMysqlArgs` call sites wrapped in `@()` |
